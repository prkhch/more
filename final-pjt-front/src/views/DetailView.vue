<template>
  <div>
      ...
      <div class="banner">
        ...
        <div v-else>
          <div class="banner-container" style="text-align: right">
            <img
              :src="getImageUrl(this.movie.poster_path)"
              class="banner-img d-block w-100"
              alt="Banner"
            />
          </div>
        ...
        <div class="banner-overlay">
          ...
          <div class="button-group">
            ...
            >
              ...
              <span v-else>
                <i
                  class="fa-regular fa-heart fa-beat fa-2xl"
                  style="color: #b40000"
                ></i>
              </span>
              <span class="like-animation" v-if="showHeartAnimation"
                ><i
                  class="fa-solid fa-heart fa-shake"
                  style="color: #b40000; font-size: 300px"
                ></i
              ></span>
            ...
            >
              ...
              <span class="check-animation" v-if="showCheckAnimation"
                ><i class="fa-solid fa-check" style="color: #00ff1e"></i
              ></span>
            </button>

            <button
              :class="{ 'transparent-button': !hasYoutubeUrl }"
    ...
    <div
      v-else
      style="background-color: black; margin-top: 50x"
      class="text-center"
    >
      ...
      <div>
        ...
        <div class="input-container">
          <textarea
            ...
            v-model="content"
            @keyup.enter="createComment(movie.id)"
            @click="clickSound()"
          ></textarea>
          <button
            type="button"
            class="btn save-btn ms-2"
            style="color: white; height: 40px"
            @click="
              createComment(movie.id);
              clickSound();
            "
          >
            <i
              class="fa-solid fa-turn-down fa-rotate-90"
          ...
          <li v-for="comment in comments" :key="comment.id" class="m-1">
            ...
            <div v-if="comment.editing" class="d-flex">
              <textarea
                ...
                @keyup.enter="modifyComment(comment)"
                @click="clickSound()"
              ></textarea>
              <button
                type="button"
...
export default {
  ...
  methods: {
      ...
      if (this.$store.state.username == null) {
        try {
          const response = await axios.get(
            `${this.$store.state.URL}/api/v1/movies/${movieId}/likecount/`
          );
    ...
    async like_movie(movieId) {
      try {
        // 비동기처리(post이후 fetchLike 순차적 호출(에러 방지))
        await axios.post(
          `${this.$store.state.URL}/api/v1/movies/${movieId}/like/${this.$store.state.username}/`
        );
    ...
    fetchisLater(movieId) {
      axios
        .get(
          `${this.$store.state.URL}/api/v1/movies/${movieId}/watchlater/${this.$store.state.username}/`
        )
        .then((response) => {
